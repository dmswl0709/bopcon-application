      ...
      <Stack direction='vertical' justifyContent='start' flexible fullHeight  fullWidth>
      {
        pageList.map((item, idx) =>
          <TouchableOpacity key={idx} style={{width: '100%'}} onPress={()=>{navigation.navigate('ContentCategoryScreen', {name: item.name, type: item.type})}}>
            <Text style={{fontSize:16, fontWeight:'bold', paddingVertical:16, paddingHorizontal: 16, width:'100%'}}>{item.name}</Text>
          </TouchableOpacity>)
        }
        ...
        <TouchableOpacity onPress={()=>{}} style={{width: '100%'}}>
          <Text style={{fontSize:16, fontWeight:'bold', paddingVertical:16, paddingHorizontal: 16, width:'100%'}}>문의하기</Text>
        </TouchableOpacity>

        <TouchableOpacity onPress={()=>{}}  style={{width: '100%'}}>
          <Text style={{fontSize:16, fontWeight:'bold', paddingVertical:16, paddingHorizontal: 16, width:'100%'}}>서비스 소개</Text>
        </TouchableOpacity>
      </Stack>
    </Layout>
  );
